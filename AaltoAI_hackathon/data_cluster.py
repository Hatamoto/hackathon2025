...
from utils import update_file_hash
...
def add_ids() -> List[Dict]:
    ...
    pathlib.Path(JSON_WITH_IDS).write_text(json.dumps(
...
def export_texts(data: List[Dict]) -> None:
    with gzip.open(TEXTS_NDJSONGZ, "wt", encoding="utf-8") as fp:
        ...
        for r in data:
            w.writerow({"entry_id": r["entry_id"],
                       "text": ascii_only(concat_text(r))})
...
def deduplicate_and_save(data: List[Dict]) -> None:
    ...
    with gzip.open(TEXTS_NDJSONGZ, "rt", encoding="utf-8") as fp:
        for obj in ndjson.reader(fp):
            entry_ids.append(obj["entry_id"])
            texts.append(obj["text"])
    n = len(texts)
    id2text = dict(zip(entry_ids, texts))
    ...
    for t in tqdm(texts, desc="Embeddings", ascii=True):
        buf.append(t)
        flush() if len(buf) >= EMB_BATCH else None
    flush()
    ...
    pathlib.Path(SRC_JSON).write_text(json.dumps(
        merged, indent=2, ensure_ascii=False), encoding="utf-8")
    print(
        f"💾  Päivitetty {SRC_JSON}:  klustereita {len(clusters_idx)} | singles {len(singles_idx)} | yhteensä {len(merged)}")
...
if __name__ == "__main__":
    try:
        ...
        # 3) Klusterit → yhdistä → päivitä SRC_JSON
        deduplicate_and_save(all_data)
        update_file_hash(SRC_JSON)  # Päivitä hash-tiedosto
        print("✅  Valmis!")
